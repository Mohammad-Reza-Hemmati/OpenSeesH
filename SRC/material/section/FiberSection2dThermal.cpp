...
FiberSection2dThermal::setTrialSectionDeformation(const Vector& deforms)
{
	...
	for (int i = 0; i < numFibers; i++) {
		...
		res += theMat->setTrial(strain, FiberTemperatures[i], stress, tangent, ThermalElongation);//***JZ
...
FiberSection2dThermal::getTemperatureStress(const Vector& dataMixed)
{
	...
	Vector DeltaThermalElong(numFibers);
	int n = dataMixed.Size() / 3;
	for (int i = 0; i < numFibers; i++) {
...
FiberSection2dThermal::determineFiberTemperature(const Vector& dataMixed, double fiberLoc)
{
	...
	int n = dataMixed.Size() / 3;
	if (fiberLoc < dataMixed[1] || fiberLoc > dataMixed[2 * n - 1])
	{
	...
	if (fabs(fiberLoc - dataMixed[2 * i + 1]) < 1e-3)
		if (i == 0)
			FiberTemperature = dataMixed[0];
		else
			FiberTemperature = dataMixed[2 * i];
	else {
		double loc1 = dataMixed[2 * i - 1];
		double loc2 = dataMixed[2 * i + 1];
		double tem1 = dataMixed[2 * i - 2];
		double tem2 = dataMixed[2 * i];
		FiberTemperature = tem1 + (tem2 - tem1) / (loc2 - loc1) * (fiberLoc - loc1);
	}
